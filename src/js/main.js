// main.js - Core functionality for Forte Card Previewer
...
(function() {
    ...
    const LIKES_REALTIME_UPDATE_INTERVAL = 30000; // Check for new likes every 30 seconds
    ...
    let likesUpdateInterval = null; // Interval for updating likes
    ...
    /** Start the real-time likes update interval */
    function startLikesUpdateInterval() {
        // Clear any existing interval first
        if (likesUpdateInterval) {
            clearInterval(likesUpdateInterval);
        }
        
        // Set up a new interval to periodically check for updates
        likesUpdateInterval = setInterval(function() {
            if (window.refreshLikesData) {
                window.refreshLikesData().then(function() {
                    // Update all thumbnails with the latest like data
                    updateAllThumbnailLikes();
                    ...
                    console.log("[Likes Update] Refreshed likes data");
                }).catch(function(error) {
                    console.error("[Likes Update] Failed to refresh likes data:", error);
                });
            }
        }, LIKES_REALTIME_UPDATE_INTERVAL);
        
        console.log("[Likes Update] Started real-time likes update interval");
    }
